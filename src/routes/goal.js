...
const { RFC3339, anyEmpty, dateString } = require(path.join(__basedir, "backend", "utils"))
    ...
    router.get("/", async (req, res) => {
        ...
        for (let goal of saving_list) {
            ...
            saving.forEach(transaction => {
                ...
                Savings.push([transaction.id, amount, dateString(dateObj), note, goal.id, RFC3339(dateObj), "savings"])
            })
        }
        ...
        for (let goal of amortization_list) {
            ...
            amortization.forEach(transaction => {
                const { date, amount, note } = transaction.data()
                const dateObj = date.toDate()
                Amortizations.push([transaction.id, amount, dateString(dateObj), note, goal.id, RFC3339(dateObj), "amortizations"])
            })
        }
        ...
        .post("/editGoal", async (req, res) => {
            ...
            const { category, name, amount, date, note } = req.body
            const user = new User(req.session.email)
            // Check to see if any field was left blank
            if (anyEmpty(category, name, amount, date)) {
                req.session.error = "Please fill out all fields"
                return req.session.save(_ => res.redirect("/goal"))
            }

            const goal = { amount: parseInt(amount), date: new Date(date), note: note }
